...
def get_music_data():
    try:
        ...
        if df.empty:
            return pd.DataFrame(columns=["name", "week", "music_url", "created_at"])

        expected_cols = ["name", "week", "music_url", "created_at"]
        for col in expected_cols:
            if col not in df.columns:
                df[col] = ""
            ...
            if gaeun_count >= 3 and sohyeon_count >= 3:
                ...
                with st.expander("📚 지금까지 올린 음악 링크 모아보기", expanded=False):
                    if week_music_df.empty:
                        ...
                        week_music_df = week_music_df.iloc[::-1]  # 최근 저장순

                        for m_idx, m_row in week_music_df.iterrows():
                            st.markdown(
                                f"**{m_row['name']}** · {m_row['created_at']}"
                            )
